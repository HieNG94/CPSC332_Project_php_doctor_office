    ...
    if (empty($_POST["specialityId"])) {
        ...
        if (!preg_match("/^[0-9]*$/", $specialityId)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                $specialityIdErr = "Speciality ID is already exists";
            } else {
                $flag1 = true;
            }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["specialityname"])) {
        ...
        if (!preg_match("/^[a-zA-Z ]*$/", $specialityname)) {
            ...
            $query = "SELECT specialityname
                            FROM speciality
                            WHERE specialityname = '" . $_POST["specialityname"] . "';";
            $result = mysqli_query($conn, $query);
            if (mysqli_num_rows($result) > 0) {
                $specialitynameErr = "Speciality name is already exists";
            } else {
                $flag2 = true;
            }
            mysqli_free_result($result);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add'])) {
        if ($flag1 == true && $flag2 == true) {
            $query2 = "INSERT INTO speciality 
                        VALUES ('" . $_POST["specialityId"] . "','" . $_POST["specialityname"] . "');";
...
<h2 style="padding-left: 50px;"><u>Add new speciality:</u></h2>

    ...
    if (empty($_POST["presciptionId"])) {
        ...
        if (!preg_match("/^[a-zA-Z0-9]*$/", $presciptionId)) {
            ...
            if (mysqli_num_rows($result) > 0) {
                $presciptionIdErr = "presciption ID is already exists";
            } else {
                $flag1 = true;
            }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($_POST["presciptionname"])) {
        ...
        if (!preg_match("/^[a-zA-Z ]*$/", $presciptionname)) {
            ...
            $query = "SELECT presciptionname
                            FROM presciption
                            WHERE presciptionname = '" . $_POST["presciptionname"] . "';";
            $result = mysqli_query($conn, $query);
            if (mysqli_num_rows($result) > 0) {
                $presciptionnameErr = "presciption name is already exists";
            } else {
                $flag2 = true;
            }
            mysqli_free_result($result);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add'])) {
        if ($flag1 == true && $flag2 == true) {
            $query2 = "INSERT INTO presciption 
                        VALUES ('" . $_POST["presciptionId"] . "','" . $_POST["presciptionname"] . "');";
...
<h2 style="padding-left: 50px;"><u>Add new presciption:</u></h2>
